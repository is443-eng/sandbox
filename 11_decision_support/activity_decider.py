...
STAGE_1_PRIORITIES = """Here are the couple's priorities:
...
STAGE_2_PRIORITIES = """Here are the couple's priorities:
...
def main() -> None:
    ...
    out_dir = Path("outputs/activity_decider")

    stage_1 = call_ollama(model, STAGE_1_PRIORITIES)
    stage_2 = call_ollama(model, STAGE_2_PRIORITIES)

    write_output(out_dir / "stage1.md", stage_1)
    write_output(out_dir / "stage2.md", stage_2)

    metadata = {
        "model": model,
        "output_files": ["stage1.md", "stage2.md"],
    }
    write_output(out_dir / "run_metadata.json", json.dumps(metadata, indent=2))
    print("Wrote outputs/activity_decider/stage1.md and stage2.md")
