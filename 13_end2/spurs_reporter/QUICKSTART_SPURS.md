...
Assumes **Python 3**, a **venv under `13_end2/spurs_reporter/.venv`** (optional), **dependencies installed** (`pip install -r 13_end2/requirements.txt` from the repo root, or `pip install -r ../requirements.txt` from `13_end2/spurs_reporter`), **Ollama running**, a **tool-capable model** pulled (e.g. `llama3.2`), and **`data/spurs_season.db`** already filled. If the DB is missing or empty, run **`python3 spurs_season_rag.py --refresh --season 2025-26`** once from this folder (needs network).
...
```bash
cd /Users/YOU/Documents/sandbox/13_end2/spurs_reporter
source .venv/bin/activate    # Windows: .venv\Scripts\activate
```

If you are already in `13_end2/spurs_reporter` in the terminal, you can skip `cd` and only run `source .venv/bin/activate`.
