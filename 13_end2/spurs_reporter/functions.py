...
# Use a tag that supports tool calling in Ollama; plain `llama3` often returns HTTP 400 when `tools` is set.
...
def _raise_for_ollama_chat(response: requests.Response) -> None:
    """Raise with Ollama JSON error body so 400s (e.g. unsupported tools) are readable."""
    try:
        ...
        if detail:
            raise requests.HTTPError(
                f"{response.status_code} Client Error for {response.url}: {detail}",
                response=response,
            ) from None
        raise
...
def ensure_ollama_available(max_wait_seconds: int = 15, poll_interval_seconds: float = 0.5) -> None:
    ...
    raise RuntimeError(
        ...
        "Start Ollama (e.g. `ollama serve`) and ensure a model is pulled.\n"
...
def agent(messages, model=DEFAULT_MODEL, output="text", tools=None, all=False):
    ...
    model : str
        The model to be used for the agent (default: DEFAULT_MODEL; use a tool-capable tag for function calling)
    output : str
    ...
    if tools is None:
        ...
        _raise_for_ollama_chat(response)
