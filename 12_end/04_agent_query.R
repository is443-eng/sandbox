...
predict_vehicle_count = function(day_of_week, hours_of_day) {
  ...
  preds = lapply(hours, function(h) {
    resp = request(ENDPOINT_URL) |>
      ...
      req_perform() |>
      resp_body_json()
    pv = resp$predicted_vehicle_count
    if (is.list(pv)) pv = unlist(pv)
    list(hour_of_day = h, predicted_vehicle_count = as.numeric(pv[[1]]))
  })
  ...
  "function" = list(
    ...
    parameters  = list(
      ...
      properties = list(
        ...
        hours_of_day = list(
          type = "array",
          description = "Vector of hours to predict (0-23), e.g. [8] or 0:23.",
...
run_brussels_query = function(user_prompt) {
  messages = list(
  ...
  list(content = final_result$content, tool_calls_n = length(tool_calls))
}

# 5. RUN OLLAMA TOOL-CALLING (two prompts per ACTIVITY_agent_query.md) ###################################

cat("--- Prompt 1: Monday 8 AM ---\n")
r1 = run_brussels_query("Predict Brussels vehicle count for Monday at 8 AM.")
cat("Agent result:", r1$content, "\n")

cat("\n--- Prompt 2: Wednesday 5 PM ---\n")
r2 = run_brussels_query("Predict Brussels vehicle count for Wednesday at 5 PM.")
cat("Agent result:", r2$content, "\n")

# 6. VERIFY ###################################

direct = predict_vehicle_count(day_of_week = 1L, hours_of_day = 8L)
cat("\nDirect API call predictions returned:", length(direct$predictions), "\n")
cat(
  "Direct API call sample (Monday 08:00):",
  direct$predictions[[1]]$predicted_vehicle_count,
  "(1m/t1)\n"
)
cat("Tool calls used (round 1):", r1$tool_calls_n, "\n")
