...
# Default: local server (start with: python runme.py from this folder).
# Override: MCP_SERVER_URL=https://... for a deployed server; set CONNECT_API_KEY if required.
...
# 3b. CALL SECOND TOOL — filter_mtcars_by_mpg ############################
print("# 3b. CALL SECOND TOOL — filter_mtcars_by_mpg ############################")

result_filter = mcp_request("tools/call", {
    "name": "filter_mtcars_by_mpg",
    "arguments": {"min_mpg": 20, "max_mpg": 25},
})

print(result_filter["content"][0]["text"])
print()
print("--- Submission: capture a screenshot of the filter_mtcars_by_mpg output above ---")
print()
    ...
    messages = [
        {
            "role": "user",
            "content": (
                "Use a tool call only. Filter the mtcars dataset to cars with mpg between 20 and 25 inclusive "
