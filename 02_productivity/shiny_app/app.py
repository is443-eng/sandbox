"""
Arcturus orbit tracker — Streamlit app.
Uses 04_arcturus_track_ecef_metrics.py to propagate and plots the track on a map.
Astranis-inspired theme: Earth Blue, Space Black, clean typography.
"""
...
# -----------------------------------------------------------------------------
# Run tracker script and return path to CSV (or None on failure)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Station-keeping metrics (GEO compliance)
# -----------------------------------------------------------------------------
...
def delta_from_slot_deg(mean_lon: float, slot_lon: float) -> float:
    ...
    d = (mean_lon - slot_lon + 180.0) % 360.0 - 180.0
    return d
...
# Try to load existing CSV from last run (session state) or from new run
...
# If we have data, show map and metrics
...
inclination_proxy_deg = ns_box_deg / 2.0  # N–S amplitude ≈ half of lat span
...
# Current position (last point) — accent
last = df.iloc[-1]
...
# ---- Side-by-side: global (2/3) | track shape (1/3 width), same height ----
MAP_HEIGHT = 420
col_global, col_shape = st.columns([2, 1])
...
# ---- Track shape (right): 1/3 width, zoom to fit track (no cutoff) ----
...
# Zoom so bounds fit with padding (Web Mercator: visible deg ≈ 360/2^z lon, 180/2^z lat)
padding = 1.12
zoom_lon = math.log2(360.0 / (lon_span * padding))
zoom_lat = math.log2(180.0 / (lat_span * padding))
zoom_fit = min(zoom_lon, zoom_lat)
zoom_fit = max(1.0, min(zoom_fit, 15.0))
lat_center = (lat_min + lat_max) / 2
lon_center = (lon_min + lon_max) / 2
view_zoomed = pdk.ViewState(
    latitude=lat_center,
...
deck_zoomed = pdk.Deck(
    layers=[path_layer, scatter_layer],
    initial_view_state=view_zoomed,
    map_style="dark",
    tooltip={"text": "Arcturus · Alt: {alt_km} km"},
)

with col_shape:
    st.subheader("Track shape")
    st.pydeck_chart(deck_zoomed, use_container_width=True, height=MAP_HEIGHT)

# -----------------------------------------------------------------------------
# Analemma (figure-8): lon vs lat, color by time of day
# -----------------------------------------------------------------------------
st.subheader("Analemma (figure-8)")
st.caption("Longitude vs latitude; color = time of day (UTC). GEO inclination + eccentricity.")
...
def hour_to_rgba(h: float) -> list[int]:
    """Map hour 0–24 to blue → cyan → yellow → blue."""
...
# PyDeck expects a list of lists for get_fill_color when per-point
ana_layer = pdk.Layer(
    "ScatterplotLayer",
    data=df_ana,
...
st.pydeck_chart(deck_ana, use_container_width=True, height=380)

# -----------------------------------------------------------------------------
# Reporting: generate and download report (Markdown)
# -----------------------------------------------------------------------------
def build_report_md(
    mean_lon: float,
    mean_lat: float,
    ...
    ew_box_deg: float,
    ns_box_deg: float,
    inclination_proxy_deg: float,
    radial_km: float,
    slot_lon: float | None,
    delta_slot: float | None,
    hours: float,
    step: int,
) -> str:
    ...
    lines = [
        ...
        "## Summary",
        f"- **Points:** {n_pts:,}",
        f"- **Mean longitude:** {mean_lon:.2f}°",
        f"- **Mean latitude:** {mean_lat:.2f}°",
        f"- **Altitude:** {alt_mean:.1f} ± {alt_std:.2f} km",
        "",
        "## Station-keeping (GEO compliance)",
        f"- **Longitude drift:** {drift_deg_per_day:.4f} °/day",
        f"- **E–W box width:** {ew_box_deg:.3f}°",
        f"- **N–S box height:** {ns_box_deg:.3f}°",
...
report_md = build_report_md(
    ...
    ew_box_deg=ew_box_deg,
    ns_box_deg=ns_box_deg,
    inclination_proxy_deg=inclination_proxy_deg,
    radial_km=radial_km,
    slot_lon=slot_lon,
